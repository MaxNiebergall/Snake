...
/*
 * Dynamic Data Structures: Dynamic data structures are used in this program in a
 * way that could not be done with finite data structures, or else would be
 * incredibly time consuming and inefficient. In this program dynamic data
 * structures are used for the snake itself. Because the snake is variable length,
 * and each piece contains data, a dynamic data structure (such as a LinkedList or
 * arrayList), is really the only way to go. Using a finite array is possible, but
 * it would be very inefficient as an array of the maximum size would need to be
 * kept in memory, regardless of the size of the snake. Dynamic Data Types: Of the
 * available dynamic data structures ArrayLists seemed to be the best choice. They
 * are interchangeable with LinkedLists in their effect, but are supposedly more
 * efficient. For this scenario Data Trees were obviously out, since there isn't any
 * data that needs comparing. If a multi-person scoreboard were to be added, a
 * Binary Data Tree could be used.
 */
...
public class Snake{
	...
	JLabel yourscoreLabel = new JLabel("Your Score: " + yourscore);
	...
	Snake(){
		...
		try{
			...
			if(fsc.hasNextLine()) highscoreLabel.setText("Highscore: " + (highscore = fsc.nextInt()));
		...
		// END OF KEY BINDINGS
		/*
		 * about.addActionListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent e) { }// end of action performed });// end of
		 * about highScores.addActionListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent e) { LinkedList<String> names = new
		 * LinkedList(); LinkedList<Integer> scores = new LinkedList(); JLabel[]
		 * nameLabels = new JLabel[10]; JLabel[] scoreLabels = new JLabel[10]; try {
		 * Scanner fsc = new Scanner(new FileReader("highScores.txt"));
		 * while(fsc.hasNextLine()&&fsc.hasNextInt()){ names.add(fsc.nextLine());
		 * scores.add(Integer.parseInt(fsc.nextLine())); } fsc.close(); } catch
		 * (FileNotFoundException e1) { e1.printStackTrace(); }
		 * frame.setVisible(false); final JFrame highScoreFrame = new JFrame();
		 * JButton goBack = new JButton("Go Back"); JPanel highScorePane = new
		 * JPanel(new BorderLayout()); for(int i=0; i<10 && i<nameLabels.length;
		 * i++){ nameLabels[i] = new JLabel(names.get(i)); scoreLabels[i] = new
		 * JLabel(""+scores.get(i)); } for(int i=0; i<10 && i<nameLabels.length;
		 * i++){ highScoreFrame.add(nameLabels[i], BorderLayout.WEST);
		 * highScoreFrame.add(scoreLabels[i], BorderLayout.EAST); } p1 123 p2 31 p3
		 * 41 p4 13 p5 132 p6 1 p7 42 p8 11 p9 0 p10 5 highScorePane.add(goBack,
		 * BorderLayout.NORTH); highScoreFrame.add(highScorePane);
		 * highScoreFrame.setSize(450, 500);
		 * highScoreFrame.setLocationRelativeTo(null);
		 * highScoreFrame.setVisible(true); goBack.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) {
		 * highScoreFrame.setVisible(false); frame.setVisible(true); }// end of
		 * action performed });// end of about try { BufferedWriter bfw = new
		 * BufferedWriter(new FileWriter("highScores.txt"));
		 * while(!names.isEmpty()){ bfw.write(names.removeFirst());
		 * bfw.write(scores.removeFirst()); } bfw.close(); } catch (IOException e1)
		 * { e1.printStackTrace(); } }// end of action performed });// end of
		 * highScores dificulty.addActionListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent e) { String level = (String)
		 * dificulty.getSelectedItem(); if(level.equals("Level 1")){ waitTime=166; }
		 * else if(level.equals("Level 2")){ waitTime=100; } else
		 * if(level.equals("Level 4")){ waitTime=80; } }// end of action performed
		 * });// end of dificulty start.addActionListener(new ActionListener() {
		 * public void actionPerformed(ActionEvent e) { reset(false); }// end of
		 * action performed });// end of start
		 */
		
		// add it all up
		scorePane.setSize(300, 50);
		...
		playingFeildPane.setSize(new Dimension(450, 250));
		scorePane.add(highscoreLabel);
		scorePane.add(new JLabel("        "));
		scorePane.add(yourscoreLabel);
		basePane.add(scorePane, BorderLayout.NORTH);
		...
		// frame settings
		frame.setTitle("Max Nieberall's Snake Game v1.0.0 "/*
																 * TODO PUT VERSION
																 * STUFF HERE
																 */);
	...
	/**
	 * extendSnake() adds a new snake object to the snake linked list, that is a
	 * copy of the last element of snake
	 */
	...
	/**
	 * updateComList() updates the component list have the correct color placement
	 * for the next frame
	 */
	...
	/**
	 * refresh() calls all the methods that move and change the snake calls the
	 * methods to update the visuals for each frame one call of refresh is
	...
	// private void youLose(){
	// int option = JOptionPane.showConfirmDialog(playingFeildPane,
